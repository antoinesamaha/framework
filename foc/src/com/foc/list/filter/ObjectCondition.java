...
public class ObjectCondition extends FilterCondition {
  ...
  public StringBuffer buildSQLWhere(FocListFilter filter, String fieldName) {
    ...
    if(operation != OPERATION_NONE){
      ...
      //Here we add the "T." only if it is not added already. Because in the Join Request case, it is added by the joins
  ...
  public int fillDesc(FocDesc focDesc, int firstID){
    ...
    if(focDesc != null){
      ...
      FObjectField objField = new FObjectField(getFieldPrefix()+"_OBJREF", "Object field", firstID + FLD_CONDITION_OBJECT, field.getFocDesc() /*,field.getKeyPrefix()*/);
